...
// The following 3 APIs are for leveraging the functionality of lazyloading ICU DATA when used in FILES_MODE.

/**
 * Initialization function for purpose of lazyloading ICU DATA when used in FILES_MODE.
 * @param callbackSignal Specifies the callback function
 *                       for notifying the path of the missing icu data file
 *                       when not found present during its loading.
 * @param completeDataListPath Specifies the path of the data list file
 *                             that contains names of all icu data files.
 *                             This param is used to check whether the name of a missing icu data file
 *                             is present in this data list and if yes,
 *                             then only callback function would be sent during its data loading.
 * @param notifyOnceForMissingFile A boolean whose value specifies whether to send
 *                                 callback function for notifying the path of
 *                                 each missing file only once (if this param is <code>true<code>)
 *                                 or allow multiple times (if this param is <code>false<code>)
 *                                 whenever the file is found not to be present during its loading.
 * @param pErrorCode An ICU UErrorCode parameter. It must not be <code>NULL</code>.
 */
U_CAPI void U_EXPORT2 udata_initForLazyloadingData(void (*callbackSignal)(const char *missingDataPath),
                                           const char *completeDataListPath, bool notifyOnceForMissingFile, UErrorCode *pErrorCode);

/**
 * This function used for clearing the resources
 * used in callback functionality for purpose of lazyloading 
 * ICU DATA when used in FILES_MODE. It is preferred to use this function during
 * termination if <code>udata_initForLazyloadingData<code> function 
 * has been called during initialization/startup. 
 * However, this function could also be used in the middle of
 * client application run as well if it is required
 * to reset to default way of not sending any callback 
 * to client when a icu data file is found missing during its loading.
 */
U_CAPI void U_EXPORT2 udata_clearForLazyloadingData();

/**
 * This function is used to check whether a 
 * callback function (to notify the path of a missing data file to client)
 * has been set for purpose of lazyloading ICU DATA when used in FILES_MODE.
 * @return true if a callback function has been set for notifying a missing icu data file.
 */
U_CAPI bool U_EXPORT2 udata_isSetCallbackForLazyloadingData();
