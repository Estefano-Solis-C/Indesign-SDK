...
/**
 * Convert string from wide character encoding to code page.
 * An intermediate conversion to UTF-16 may be performed.
 * 
 * @param cnv The converter that is used to convert from UTF-16 to code page.
 * @param src	Wide character input string
 * @param src_len Length of input string in wchar_t units; length of -1 indicates null-terminated string.
 * @param dest Buffer for multibyte output string 
 * @param dest_size Capacity of output buffer on input; length of converted string on return
 * @param err AGL error code
 * @return Converted string on success; NULL on failure
 *
 */
U_CAPI char* U_EXPORT2
ucnv_wcstombs(UConverter* cnv, const wchar_t* src, int32_t src_size, char* dest, int32_t* dest_size, UErrorCode* err);

/**
 * Convert string from code page to wide character encoding.
 * An intermediate conversion to UTF-16 may be performed.
 *
 * @param cnv The converter that is used to convert from code page to UTF-16.
 * @param src	Input string in code page encoding
 * @param src_len Length of input string in bytes; length of -1 indicates null-terminated string.
 * @param dest Buffer for output wide character string 
 * @param dest_size Capacity of output buffer on input; length of converted string on return
 * @param err AGL error code
 * @return Converted string on success; NULL on failure
 *
...
ucnv_mbstowcs(UConverter* cnv, const char* src, int32_t src_size, wchar_t* dest, int32_t* dest_size, UErrorCode* err);
