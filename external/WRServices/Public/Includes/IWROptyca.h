...
enum RoutingScripts
{
	...
	kWRMENAOnly = 4,
	kWRCJKOnly = 8,
	kWRNoScriptLipikaOnly = 16, // If no scripts are detected route everything via Lipika
};
...
public:
	...
	*/
    ...
    virtual WRFloat             GetTrackingAdjustment(StyleRef style) { return 0; };
	...
	virtual WRInt32				GetAutoTextATE(StyleRef style, WRUnichar32 ch, WRUnichar32* text, WRInt32 maxLen) { return 0; }
	virtual WRFloat				CalculateWidth(StyleRef style, WRInt32 index) = 0;
	// WR-1131 : let derived class override the function, added defaultimpl
	virtual WRFloat				GetLeftEdgeOpticalAdjustment(StyleRef style, WRGlyphID32 gid) { return WRFloat(0.0); }
	virtual WRFloat				GetRightEdgeOpticalAdjustment(StyleRef style, WRGlyphID32 gid) { return WRFloat(0.0); }
    virtual WRFloat             GetLeftHang(StyleRef style, WRUnichar32 ch) { return WRFloat(0.0); } //Get hanging punctuation values from Client for Roman/Burasagari.
    virtual WRFloat             GetRightHang(StyleRef style, WRUnichar32 ch) { return WRFloat(0.0); }
	...
	*/
    ...
    // replicating kerning type from ATE side . Used to disallow optical kerning in case of vertical feature
    enum class WRKernType { kWRNoAutoKern, kWRMetricKern, kWROpticalKern, kWRMetricRomanOnlyKern };
    virtual WRKernType                GetKerningType(StyleRef style) = 0;
    ...
    virtual BaselineDirection GetBaselineDirection(StyleRef style)=0;
    ...
    virtual GlyphOrientation GetGlyphOrientation(StyleRef style)=0;
	virtual void GetTsumeAdjustments(StyleRef style, WRGlyphID32 gid, WRUnichar32 charCode, WRFloat& leftAdj, WRFloat& rightAdj) = 0;
	virtual bool GetMojiKumiAdjustment(StyleRef style1, StyleRef style2, bool atLineStart, bool atLineEnd, bool atParaStart, bool atParaEnd, WRUnichar32 prevChar, WRUnichar32 currChar, WRUnichar32 nextChar, WRFloat& leftAdj, WRFloat& rightAdj, WRFloat& nextLeftAdj,bool currFullWidth, bool nextFullWidth, bool horizontalOrientation,bool isSSITS) = 0;
    virtual void GetAkiAdjustments(StyleRef style, WRGlyphID32 gid, WRUnichar32 charCode, WRFloat& leftAki, WRFloat& rightAki)
    {
        leftAki = WRFloat(0.0);
        rightAki = WRFloat(0.0);
    ...
    virtual int GetBurasagariType(StyleRef style) = 0;
	virtual StyleRef GetEndOfLineHyphenStyle(WRInt32 charIndex, StyleRef originalStyle) { return originalStyle; }
	virtual StyleRef GetStartOfLineHyphenStyle(WRInt32 charIndex, StyleRef originalStyle) { return originalStyle; }
    virtual bool ContainsAutoTsumeAdjustments(StyleRef style, WRUnichar32 charCode)
    {
        return false;
    }
};
...
public:
	...
	virtual void SetClientEncoding(ClientEncoding enc) = 0;
	virtual void SetEncodingMapUsed(bool) = 0;
	
	/*This method is used to set the scripts to be routed through traditional Lipika workflow.
	Use enum RoutingScripts
	{	kWRLatinOnly = 1,
		kWRIndicOnly = 2,
		kWRMENAOnly = 4,
	};
	For example: 
	In order to set ony Latin to route through Lipika pass kWRLatinOnly.
	In order to set Latin and Indic only to route through Lipika the client has to pass (kWRLatinOnly | kWRIndicOnly). This ensures that the text which contains only a combination of Indic and Latin scripts will pass through Lipika.
	*/
	virtual void SetRoutingScriptsForLipika(WRInt8 scripts) = 0;
	
	virtual void SetWritingDirection(WritingDirection dir) = 0;
	virtual ShapingEngine GetShapingEngine() = 0;
	virtual ClientEncoding GetClientEncoding() = 0;
	virtual bool GetEncodingMapUsed() = 0;
    virtual void SetLineStartEndInfo(bool isFirstLine, bool isLastLine) = 0;
    ...
    */
    ...
    virtual WRFloat* GetLeftAdjustments() = 0;   // Needed to fetch Tsume, Mojikumi, Aki values applied by WR.
    virtual WRFloat* GetRightAdjustments() = 0;
    ...
    */
	...
	@return a pointer to a InputOutputHB . client is responsible for cleaning object.

    this API will be deprecated in future major version . Client should instead use another API GetHarfbuzzMappingPTR() to fetch pointer and store it and clear the memory when not needed .
	*/
   ...
   */
    ...
    virtual const char* GetHBVersion() = 0;
    
    virtual WRFloat GetTargetWidth() = 0;
    ...
    */
	...
	virtual void				PrepareGlyphRuns_HB(bool doReodering) = 0;
	...
	**/
	...
	virtual WRFloat getJustificationAdjustmentForTCY()=0;
	virtual WRGlyphID32 GetCachedGlyphId(WRFontDict* dict, const WRVector<float>& fWRDesignVector, WRInt32 dir, WRUnichar32 unicode) = 0;
	virtual void SetCachedGlyphId(WRFontDict* dict, const WRVector<float>& fWRDesignVector, WRInt32 dir, WRUnichar32 unicode, WRGlyphID32 gid) = 0;
	virtual WRFloat GetCachedGlyphAdv(WRFontDict* dict, const WRVector<float>& wrdesignVector, WRInt32 dir, WRGlyphID32 gid) = 0;
	virtual void SetCachedGlyphAdv(WRFontDict* dict, const WRVector<float>& wrdesignVector, WRInt32 dir, WRGlyphID32 gid, WRFloat adv) = 0;
...
protected:
	IWROptyca*						fNext;
public:
    /// Returns a pointer to the Harfbuzz mapping log
	/**
	This method returns a pointer to the internal mapping object containing input-output maping from Harfbuzz
	@return a pointer to a InputOutputHB.
	
    Client is responsible for freeing up memory and should use only one version of GetHarfbuzzMapping api , one API return ref (GetHarfbuzzMapping) and another pointer (GetHarfbuzzMappingPTR )
	using both API will cause the issue
	*/
    virtual InputOutputHB*	GetHarfbuzzMappingPTR() = 0;

	/// Returns a pointer to the newly created Harfbuzz object
   /**
   This method returns a pointer to the newly created harfbuzz mapping object from given state 

   Client is responsible for freeing up memory
   */

	virtual InputOutputHB* CreateHarfbuzzMapping(WRInt32				encodedInputCount, //Encoded input count
		WRInt32			    inputCount,//client's original character input count
		WRInt32				outputCount, ///< number of output slots (glyphs)
		const HBRunList& inputRuns,///track of input runs
		const HBRunList& outputRuns,///track of output runs
		const WRVector<WRInt32>& inputClusters,// keep track of cluster values of input characters before shaping
		const WRVector<WRFloat>& glyphPositions,//keep track of cummulative x-advances in case of horizontal direction/y-advances of glyphs in case of vertical direction;  for 0th glyph its glyphPos to be assumed 0
		const WRVector<WRInt32>& updatedInputClusters,// keep track of cluster values of input characters after shaping
		const WRVector<WRInt32>& outputClusters,// keep track of cluster values of output glyphs
		const WRVector<WRInt32>& autoTextInternalToClient,//stores mapping of fInputCount to fClientInputCount
		const WRVector<WRInt32>& autoTextClientToInternal,//stores mapping of fInputCount to fClientInputCount
		const std::map<WRInt32, WRAutoTextRange>& autoTextMapping,//fClientInputCount to WRAutoTextRange of internal inputCount
		const bool isRTL,
		const WRVector<WRInt32>& utfMapping,
		const HarfBuzzRunList& inputHarfBuzzRunList,
		const HarfBuzzRunList& outputHarfBuzzRunList,
		const std::map<int, HBClustersPTR>& clusterInfo) = 0;
    virtual WRVector<WRFloat>&     GetKernXoffsets() = 0;
    virtual WRVector<WRFloat>&     GetKernYoffsets() = 0;
   };
