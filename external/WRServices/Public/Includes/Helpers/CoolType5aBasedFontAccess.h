...
public:
	...
	void SetCTContext(BIB_NAMESPACE::CTFontSet* context) { fCTContext = context; }
	...
	{
		BIB_NAMESPACE::CCTFontDict fontDict((BIB_NAMESPACE::CTFontDict*) aFont);
		BIB_NAMESPACE::CCTEncoding cctEnc((BIB_NAMESPACE::CTEncoding*)encoding);
		BIB_NAMESPACE::CCTFontInstance cctInst(fontDict, *(BRVCoordMatrix*)matrix ,(CTTextWritingDirection)writingDirection, cctEnc, mmDesignVector);
		BIB_NAMESPACE::CTFontInstance* ctInst = cctInst.GetPointerAndAddRef();
		return (WRFontInstance*)ctInst;
	}

	virtual void WRDeleteFontInstance(WRFontInstance *inst)
	{
		BIB_NAMESPACE::CCTFontInstance cctInst((BIB_NAMESPACE::CTFontInstance*) inst,true);
	}

	virtual WREncoding*	WRGetPreDefinedEncoding( WRInt32 whichEncoding )
	{
		BIB_NAMESPACE::CCTEncoding cctEnc = BIB_NAMESPACE::CCTEncoding(CTPreDefinedEncodingType (whichEncoding));
		BIB_NAMESPACE::CTEncoding* ctEnc = &(*cctEnc);
		return (WREncoding*)ctEnc;
	}

	virtual void WRGetGlyphWidth( WRFontInstance *inst, WRGlyphID32 glyphID, WRFloat* width)
	{ 
		BIB_NAMESPACE::CCTFontInstance cctInst((BIB_NAMESPACE::CTFontInstance*) inst);
	...
	{ 
		BIB_NAMESPACE::CCTFontInstance cctInst((BIB_NAMESPACE::CTFontInstance*) inst);
			...
			if (antiAliasType >=0)
			{
				BIB_NAMESPACE::CTFontDict* pDict = (BIB_NAMESPACE::CTFontDict*)cctInst.GetFont();
				bool bHintableScript = false;
				std::map<BIB_NAMESPACE::CTFontDict*,bool>::const_iterator it = fMapHint.find(pDict);
				if (it == fMapHint.end())
				{
					BIB_NAMESPACE::CCTFontDict fontDict(pDict);
					...
					fMapHint.insert(std::pair<BIB_NAMESPACE::CTFontDict*, bool>(pDict, bHintableScript));
				}
				else
	...
	{ 
        ...
        virtual void WRGetGlyphEmBox( WRFontInstance *inst,WRFloatRect* bbox )
    {
		BIB_NAMESPACE::CCTFontInstance cctInst((BIB_NAMESPACE::CTFontInstance*) inst);
		BIB_NAMESPACE::CCTFontDict pDict = cctInst.GetFont();
        BRVRealCoordRect brvRect;

        pDict.GetIdeoEmBox(brvRect);
    bbox->left = brvRect.xMin;
    bbox->top = brvRect.yMin;
    bbox->right = brvRect.xMax;
    bbox->bottom = brvRect.yMax;
	...
	{
		CBRVCoordMatrix mtx;
		BIB_NAMESPACE::CCTFontDict font((BIB_NAMESPACE::CTFontDict*)aFont);
		BIB_NAMESPACE::CCTEncoding cctEnc((BIB_NAMESPACE::CTEncoding*)encoding);
 
		BIB_NAMESPACE::CCTFontInstance cctInst( font, mtx, (CTTextWritingDirection) writingDirection, cctEnc);
		BIB_NAMESPACE::CTFontInstance* ctInst = cctInst;
											...
											WRRealGlyphPoint* strikes, WRInt32* numStrikesAvail )
	{
        ...
        *numStrikesAvail = numGIDs;
        ...
        int maxGlyphID = WRGetNumGlyphs((WRFontDict*)((BIB_NAMESPACE::CTFontDict*)cctInst.GetFont())) - 1;
        WRGlyphID32 notDefGlyphID = WRGetNotDefGlyphID((WRFontDict*)((BIB_NAMESPACE::CTFontDict*)cctInst.GetFont()));
...
//		return ::CTTranslateGlyphID( (CTTranslator*) trans, glyphID, toString, toStrLen, matchPolicy );
	...
	virtual WRBool WRGetFontName(WRFontDict *aFont, char* buffer, int avail)
	{
			...
			BIB_NAMESPACE::CCTFontDict font((BIB_NAMESPACE::CTFontDict*)aFont);
			BIB_NAMESPACE::CBIBStringAtom strAtom = font.GetFontName();
        	if ( ! strAtom.IsValid() )
				return false;
	...
	virtual WRBool WRGetNthOTVDesignAxisName(WRFontDict *aFont, int index , char* buffer)
	{
			BIB_NAMESPACE::CCTFontDict font((BIB_NAMESPACE::CTFontDict*)aFont);
			CTLocaleID locale = en_US;
			CTUTF16Unit axisName[16];
			size_t axisNameLen = 16;
			unsigned char(*axisTag)[4] = (unsigned char(*)[4])buffer;

			return font.GetNthOTVDesignAxisName(index, locale, axisName, axisNameLen, axisTag);
	...
	{
		BIB_NAMESPACE::CCTFamilyList tmpFamily( fCTContext, kCTASCIIEnum);
		BIB_NAMESPACE::CTFamilyList* ctFam = tmpFamily.GetPointerAndAddRef();
		return (WRFamilyList*) ctFam;
	}

	virtual	void	WRReleaseFamilyList( WRFamilyList* famList)
	{
		BIB_NAMESPACE::CTFamilyList* ctFamList = (BIB_NAMESPACE::CTFamilyList*)famList;
		if (ctFamList)
        {
#if defined(NO_DYNAMIC_BINDING)
                      _CTBaseReleaseProc_Release(ctFamList);
#else
                      BIB_NAMESPACE::gCTBaseInterface.Release(ctFamList);
#endif
        }
	}
	...
	{
		BIB_NAMESPACE::CCTFamily cctFam((BIB_NAMESPACE::CTFamily*)family);
		BIB_NAMESPACE::CCTFontDict cctDict = cctFam.GetNthFont(index);
		BIB_NAMESPACE::CTFontDict* ctDict = cctDict;
		return (WRFontDict*)ctDict;
	}

	virtual WRInt32 WRGetNumFamilies(WRFamilyList* famList)
	{
		BIB_NAMESPACE::CCTFamilyList tmpFamily( (BIB_NAMESPACE::CTFamilyList*) famList);
	...
	{
		BIB_NAMESPACE::CCTFamilyList cctFamList( (BIB_NAMESPACE::CTFamilyList*) famList);
		BIB_NAMESPACE::CCTFamily cctFam = cctFamList.GetNthFamily(index);
		BIB_NAMESPACE::CTFamily* ctFam = cctFam.GetPointerAndAddRef();
		return (WRFamily*) ctFam;
	}

	virtual	void	WRReleaseFamily( WRFamily* family)
	{
		BIB_NAMESPACE::CTFamily* ctFamily = (BIB_NAMESPACE::CTFamily*)family;
		if (ctFamily)
        {
#if defined(NO_DYNAMIC_BINDING)
                      _CTBaseReleaseProc_Release(ctFamily);
#else
					  BIB_NAMESPACE::gCTBaseInterface.Release(ctFamily);
#endif
        }
	}
	...
	virtual void WRGetHorizontalMetrics(WRFontDict *aFont,WRFloat& ascent, WRFloat& descent, WRFloat& lineGap, WRFloat* mmDesignVector=NULL, int designVectorLength=0)
	{
			BRVRealCoord typoAscent;
	        BRVRealCoord typoDescent;
	        BRVRealCoord typoLineGap;
			BIB_NAMESPACE::CCTFontDict fontDict((BIB_NAMESPACE::CTFontDict*)aFont);
			if (designVectorLength)
			{
				...
				fontDict.GetHorizontalMetrics(typoAscent, typoDescent, typoLineGap , mmDesignVector, designVectorLength);
				free(defaultVector);
			}
			else
			{
	...
	{
		BIB_NAMESPACE::CCTFontDict cctFont((BIB_NAMESPACE::CTFontDict*)aFont);
	...
	{
			BIB_NAMESPACE::CCTEncoding  cctEnc ((BIB_NAMESPACE::CTEncoding*) toEncoding);
			BIB_NAMESPACE::CCTFontDict cctFont((BIB_NAMESPACE::CTFontDict*)aFont);
			BIB_NAMESPACE::CCTTranslator trans( cctEnc, cctFont);
	        return (WRTranslator*) trans.GetPointerAndAddRef();
	}

	virtual void			WRDeleteTranslator( WRTranslator* trans )
	{
			BIB_NAMESPACE::CCTTranslator cctTrans( (BIB_NAMESPACE::CTTranslator*)trans, true);
	}
	...
	virtual char			WRTranslateEncoding( WRTranslator* trans, const char* fromString,
												...
												WRInt32 matchPolicy )
	{
		BIB_NAMESPACE::CCTTranslator cctTrans( (BIB_NAMESPACE::CTTranslator*)trans);
        ...
        *bytesWritten = *toStrLength;
        ...
        bool succeeded = cctTrans.TranslateEncoding((BIB_NAMESPACE::CTEncoding*) fromEncoding, (const unsigned char*)fromString, *fromStrLength, fromBytesRead,
	...
	{
			BIB_NAMESPACE::CCTEncoding cctEnc( (BIB_NAMESPACE::CTEncoding*) encode, true );

	}
    virtual CTGlyphID WRCFFGlyphIDToCTGlyphID(WRFontDict * aFont, WRGlyphID32 cffID)
    {
        // return CTGlyphID incase of CFF fonts. Needs updated hb_ct_get_nominal_glyph HarfbuzzCooltype.cpp
		BIB_NAMESPACE::CCTFontDict font((BIB_NAMESPACE::CTFontDict*)aFont);

        if ((font.IsCFF()) &&
            (!font.CTGlyphIDIsCFFGlyphID()))
        {
            // CFFGlyphIDToCTGlyphID takes 16bit intput. Might have truncation issues.
            CTGlyphID CTglyphID = font.CFFGlyphIDToCTGlyphID((BRVUns32)cffID);
            return CTglyphID;
        }
        return cffID;
	...
	{
		BIB_NAMESPACE::CCTEncoding cctEncoding;
		BIB_NAMESPACE::CTEncoding* ctEnc = cctEncoding.GetPointerAndAddRef();
		return (WREncoding*)ctEnc;
	}

	virtual WREncoding*	WRNewPlatformEncoding( WRCodePage cp )
	{
		BIB_NAMESPACE::CCTEncoding cctEnc = BIB_NAMESPACE::CCTEncoding(CTCodePage(cp));
		BIB_NAMESPACE::CTEncoding* ctEnc = cctEnc.GetPointerAndAddRef();
		return (WREncoding*)ctEnc;
	}
	...
	virtual void			WRGetICFBox( WRFontDict *aFont, WRFloatRect *icfBox, WRFloat* mmDesignVector = NULL, int designVectorLength = 0)
	{
			BIB_NAMESPACE::CCTFontDict font((BIB_NAMESPACE::CTFontDict*)aFont);
			BRVRealCoordRect box;
		if(designVectorLength)
		{
			int numDesignAxes = static_cast<int>(font.GetNumDesignAxes());
			BRVRealCoord *defaultVector = (BRVRealCoord*)malloc(numDesignAxes * sizeof(BRVRealCoord));
			//checks to make sure that valid design vector is passed in FontInstance Creation
			if (designVectorLength < numDesignAxes)
			{
				font.GetDefaultDesignVector(defaultVector);
				for (int i = 0; i < designVectorLength; i++)
				{
					defaultVector[i] = mmDesignVector[i];
				}
			...
			font.GetIdeoEmBox(box, mmDesignVector, designVectorLength);
			free(defaultVector);
		}
		else
		{
			font.GetIdeoEmBox(box);
		}
	...
	{
		BIB_NAMESPACE::CBIBStringAtom enc;
		size_t designVecLength; 
		BIB_NAMESPACE::CCTFontDict cctFontDict = fCTContext.FindFont(name, enc, NULL, designVecLength);
		if (!cctFontDict.IsValid() || cctFontDict.GetFontStatus() == kCTFontStatus_Disabled || cctFontDict.GetFontStatus() == kCTFontStatus_Invalid || cctFontDict.GetNumGlyphs() == 0) 
			return NULL;
		BIB_NAMESPACE::CTFontDict* ctDict = cctFontDict;
		return (WRFontDict*)ctDict;
	}
	...
	virtual WRFontDict*	WRSearchFont(const char* familyName, const char* styleName, int& matchQuality)
	{
		...
		size_t ucLen = 256;
		size_t avail = 256;
		BIB_NAMESPACE::CTFontDict* resDict = NULL;
		BIB_NAMESPACE::CCTFamilyList cctFamList( fCTContext, kCTASCIIEnum);
		matchQuality = 4;
		size_t numFamilies = cctFamList.GetNumFamilies();
		for (size_t i = 0; i < numFamilies && matchQuality > 2; i++)
		{
			BIB_NAMESPACE::CCTFamily cctFam = cctFamList.GetNthFamily(i);
			size_t numFont = cctFam.GetNumFonts();
			for (size_t j = 0; j < numFont; j++)
			{
				BIB_NAMESPACE::CCTFontDict cctDict = cctFam.GetNthFont(j);
		...
		WRInt32 writingDirection,int designVectorLength)
	{
		...
		BIB_NAMESPACE::CCTEncoding cctEnc((BIB_NAMESPACE::CTEncoding*)encoding);
		BIB_NAMESPACE::CCTFontInstance cctInst(fontDict, *(BRVCoordMatrix*)matrix, (CTTextWritingDirection)writingDirection, cctEnc, mmDesignVector,designVectorLength);
		free(defaultVector);
		BIB_NAMESPACE::CTFontInstance* ctInst = cctInst.GetPointerAndAddRef();
		return (WRFontInstance*)ctInst;
	}
...
protected:
	...
	BIB_NAMESPACE::CCTFontSet					fCTContext;
	std::map<BIB_NAMESPACE::CTFontDict*,bool>	fMapHint;
