...
#ifndef WR_DISABLE_AGM
using WRCTSharedBezierPath = BIB_NAMESPACE::CAGMSharedBezierPath;
using WRCTGlyphInfo = AGMGlyphInfo;
#else
using WRCTSharedBezierPath = CCTSharedBezierPath;
using WRCTGlyphInfo = CCTGlyphInfo;
#endif
...
)
{
		...
		)
	{
		...
		{
			BIB_NAMESPACE::CCTFontInstance fontInstance((BIB_NAMESPACE::CTFontInstance*)cntxt->fontInstance);
			BIB_NAMESPACE::CCTFontDict fontDict(fontInstance.GetFont());
...
{	
	BIB_NAMESPACE::CCTFontInstance fontInstance((BIB_NAMESPACE::CTFontInstance*)inst);
	WRUInt32 i;
	...
	BIB_NAMESPACE::CTFeatureInfo info(
...
{	
	...
	BIB_NAMESPACE::CTTextRunProcs runProcs(GetGlyphIDAt,
							ReplaceOneByOne,
							ReplaceOneByMany,
							ReplaceRangeByOne,
							ReplaceManyByOne,
...
bool CoolType5aBasedFontAccess::GetFontVersion(WRFontDict *fntDict, WRUInt8 *versionStr, WRByteCount len)
{
	...
	*versionStr = 0;
	...
	BIB_NAMESPACE::CCTFontDict ctFontDict((BIB_NAMESPACE::CTFontDict*)fntDict);
...
{ 
	BIB_NAMESPACE::CCTFontDict cFont( (BIB_NAMESPACE::CTFontDict*) fntDict );
	...
	BIB_NAMESPACE::CCTText textOutlines(cFont, cMatrix, cGlyphBuf);

	WRCTSharedBezierPath cOutlines;
	...
	textOutlines.GetOutlines( &devMatrix, clipB, BIB_NAMESPACE::kCTTextPreferOutlines, cOutlines);

	size_t  pathNumPoints; 
	size_t  pathNumGlyphs; 
	const BRVRealCoord* pathCoords; 
	const AIMPointType* pathTypes;
	const WRCTGlyphInfo* pathGlyphPositions;
	size_t  pathCurPos = 0; 
	...
	for (pathCurType = 0; pathCurType < pathNumPoints; pathCurType++)
	{
		...
		{ 
			case kAIMPathPtClose:
				pathCurPos+=2;
				break; 
			case kAIMPathPtMoveTo:
