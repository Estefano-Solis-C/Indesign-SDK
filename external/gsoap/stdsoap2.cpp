...
again:
  ...
  // Watson#2724090-some Mac users report unhandled SIGPIPEs during our 'send'
  // We set SO_NOSIGPIPE socket option to suppress SIGPIPEs on Mac
#ifdef MAC_OSX
  if (setsockopt(fd, SOL_SOCKET, SO_NOSIGPIPE, (char*)&set, sizeof(int)))
  { soap->errnum = soap_socket_errno(fd);
    soap_set_sender_error(soap, tcp_error(soap), "setsockopt SO_NOSIGPIPE failed in tcp_connect()", SOAP_TCP_ERROR);
    soap->fclosesocket(soap, fd);
#ifdef WITH_IPV6
    freeaddrinfo(ressave);
#endif
...
// For some bizzare reason, Windows defaults to IPV6_V6ONLY = TRUE - i.e. if the gSOAP code is compiled with '-DWITH_IPV6', then
//	IPv4 addresses will not be accepted. In Vista/Longhorn, MS fixed their IPv6 implementation to give a way to change this 
//	behavior (by setting the socket option IPV6_V6ONLY to false as we are doing here), but at least in VisualStudio8, that 
//	#define isn't available. This will *not* work with MS's IPv6 implementations in Windows 2003 Server or earlier. Passing the full 
//	IPv6 address will work in all cases; passing a IPv4 address will work in Vista/Longhorn and Windows 2003 Server if IPv6 isn't installed.
//	This is apparently a known gSOAP bug (http://sourceforge.net/tracker/index.php?func=detail&aid=2059623&group_id=52781&atid=468021),
//	and will be addressed by making this change in an 'upcoming release'; this hack can go away then. -brendan 9/4/08
//
#define IPV6_V6ONLY	27
	setsockopt(soap->master, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&ipv6Only, sizeof(int));	
	}
#endif
#endif
...
/* Use soap->host to store the numeric form of the remote host */
		...
		// We set SO_NOSIGPIPE socket option to suppress SIGPIPEs on Mac
#ifdef MAC_OSX
		if (setsockopt(soap->socket, SOL_SOCKET, SO_NOSIGPIPE, (char*)&set, sizeof(int)))
        { soap->errnum = soap_socket_errno(soap->socket);
          soap_set_receiver_error(soap, tcp_error(soap), "setsockopt SO_NOSIGPIPE failed in soap_accept()", SOAP_TCP_ERROR);
          soap_closesock(soap);
          return SOAP_INVALID_SOCKET;
        }
#endif
