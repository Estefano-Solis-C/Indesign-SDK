...
// [LDM] start manual change to header
#if defined (__GNUC__)
# define MAC_OSX
#endif
// [LDM] end
...
// InDesign's definition of DEBUG in the debug build triggers gSoap to operate in debug mode
// and it appears like there is a small memory leak with the names of the logs "RECV.log", "SENT.log" and "TEST.log"
// If you define DEBUG below to turn on gSoap's debugging, be aware that you might see these leaks. Boctor 6/17/04
// This was true of gSoap 2.4 but has not been validated with gSoap 2.7c Millett 10/05/04
// Appears to be true with gsoap 2.7.9f; in any case, we don't really want all the log files generated by default in debug
#ifdef DEBUG
	#undef DEBUG
#endif
